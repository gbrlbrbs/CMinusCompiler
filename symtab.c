  ...
  { if (hashTable[j] != NULL)
    { BucketList l = hashTable[j];
      ...
      { LineList t = l->lines;
        ...
        if (node->node_kind == Decl) {
          switch (node->kind.decl) {
            case Fun:
              fprintf(listing, "Function  ");
              break;
            case Var:
              if (node->type == Int) fprintf(listing, "Variable  ");
              else if (node->type == Array) fprintf(listing, "Array Var.");
              break;
            case Param:
              if (node->type == Int) 
...
{ int i;
  ...
  for (i = 0; i < nScope; ++i) {
    ...
    if (i == 0) {     // global scope
      ...
      fprintf(listing, "function name: %s ", scope->funName);
    }
