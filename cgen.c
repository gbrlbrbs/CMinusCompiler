...
static int getBlockOffset(TreeNode *list) {
    ...
    if (list == NULL) {
        // offset = 0;
    } else if (list->node_kind == Decl) {
        /* local variable declaration */
        TreeNode *node = list;
        while (node != NULL) {
          switch (node->kind.decl) {
            case Var:
              if (node->type == Array) offset += node->child[0]->attr.val;
              else ++offset;
              break;
            case Param:
              ++offset;
...
/* Procedure genStmnt generates code at a statement node */
static void genStmnt(TreeNode *tree) {
    TreeNode *p1, *p2, *p3;
...
static void genExp(TreeNode *tree, int lhs) {
    ...
    switch (tree->kind.exp) {
        ...
        case ArrId:
            ...
            if (tree->kind.exp == ArrId) {
                /* kind of node is for array id */

                if (loc >= 0 && loc < numOfParams) {
                    /* generate code to push address */
                    emitRO("ADD", ac, mp, ac,
                           "id: load the memory address of base address of "
                           "array to ac");
                    emitRO("LD", ac, 0, ac,
                           "id: load the base address of array to ac");
                } else {
                ...
                emitRO("SUB", ac, ac1, ac,
                       "id: calculate element address with index");
            } else {
        ...
        case Call:
            ...
            if (strcmp(tree->attr.name, "input") == 0) {
                ...
                emitRM("ST", mp, localOffset + ofpFO, mp,
                       "call: store current mp");
                ...
                emitRM("LD", pc, loc, gp,
                       "call: relative jump to function entry");
...
static void genDecl(TreeNode *tree) {
    ...
    switch (tree->kind.decl) {
        case Fun:
            ...
            emitRM("ST", ac1, loc, gp,
                   "func: store the location of func. entry");
            ...
            if (strcmp(tree->attr.name, "main") == 0) mainFuncLoc = funcBodyLoc;
...
void genMainCall() {
    ...
    emitRM("LDC", pc, mainFuncLoc, 0,
           "call: unconditional jump to main() entry");
